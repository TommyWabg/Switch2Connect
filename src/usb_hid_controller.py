...
# True once a client's read loop is streaming input. While set, the pyusb command path is
# off limits: reaching interface 1 through libusb requires touching the whole composite
# device, and SET_CONFIGURATION on it resets the endpoints -- killing the interface 0 input
# stream we are in the middle of reading. See send_pro_controller2_usb_command().
...
def _pro2_winusb_interface_guids() -> list[str]:
    ...
    if not guids and not _winusb_binding_warned:
        ...
        # This is the single biggest environment split for the wired pad. On a stock
        # Windows the MS OS 2.0 descriptors get interface 1 bound to WinUSB automatically;
        # on images with driver auto-install disabled (IoT / Enterprise LTSC, WSUS-managed
        # fleets) it never happens. Without it every vendor command has to fall back to the
        # HID output-report path, which is known to disturb the input stream -- so make the
        # condition explicit in the log instead of failing silently.
        logger.warning(
            "Wired USB: interface 1 (MI_01) is not bound to WinUSB -- vendor command "
            "endpoint unavailable. Services found: %s. Startup/rumble commands will use "
            "the HID output-report fallback, which is less reliable. Installing the WinUSB "
            "driver for MI_01 (see drivers/) is recommended.",
            services_seen or "(MI_01 not present in the registry)")
    return guids
...
def _ensure_configuration(dev) -> None:
    ...
    rumble fallback used to, every 0.5 s) tears down the input stream over and over, which
    is what made the pad drop within seconds and refuse to come back.
    """
    try:
...
def send_pro_controller2_usb_command(command: bytes) -> bool:
    ...
    # Never reach for libusb while the HID input stream is live -- see _usb_input_streaming.
...
def initialize_pro_controller2_usb_reports() -> str:
    ...
    # Blocked while input is streaming: the pyusb route has to go through the composite
    # device and can reset the interface 0 endpoints we are reading from.
    if _usb_input_streaming.is_set():
        logger.debug("Wired USB init: skipping pyusb path while input is streaming")
        return ""
...
class USBHidController(Controller):
    ...
    async def initialize(self):
        ...
        if self._init_transport in ("hid_fallback", "none"):
            logger.warning(
                "Wired USB init transport = %s (%s): the vendor command endpoint is not "
                "reachable on this system, so the controller may be less stable. See the "
                "MI_01/WinUSB warning above.", self._init_transport, self.device.address)
        else:
