...
if ($viGEmApps) {
    ...
    if ($anyUninstalled) {
        Write-Host "Official uninstallation complete! Skipping manual cleanups to prevent service and registry conflicts." -ForegroundColor Green
        Write-Host "A system reboot is highly recommended." -ForegroundColor Green
        Exit 0
    }
} else {
