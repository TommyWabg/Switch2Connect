...
* **In-App Gyro Trigger Deadzone:** Configurable within the In-App Gyro mapping pop-up window. Assign a dedicated button to completely pause gyro control for 100ms upon press or release. Additionally, a configurable Trigger Deadzone slider value is applied as the In-App Gyro deadzone setting while the button is held and for 200ms after its release. This effectively prevents unintended cursor movement or instability caused by controller vibration during button presses.
* **In-App Gyro Trigger Dampening:** Configurable within the In-App Gyro mapping pop-up window. Assign any button to proportionally reduce gyro sensitivity by a customizable percentage. This dampening effect remains active while the button is held and for 200ms after its release, ensuring precise and stabilized aiming during critical moments without abrupt sensitivity changes.
