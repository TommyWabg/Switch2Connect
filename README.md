...
* **USB Connected Switch 2 Pro Controller Support:** Added wired support for the Nintendo Switch 2 Pro Controller via USB connection.
  
  ***Note: The vibration functionality of the USB-connected Switch 2 Pro Controller heavily relies on the power delivery capabilities of your USB port and cable. Insufficient power supply may result in vibration failure or unexpected controller disconnections.***

* **HidHide Driver Integration for Double Input Prevention:** Introduced native support for the HidHide kernel driver to hide the physical wired Switch 2 Pro Controller from other applications, effectively eliminating the "Double Input" issue where games detect both the physical and emulated virtual controllers.
  * **Automated Workflow:** The application automatically detects when a wired Switch 2 Pro Controller is plugged in and will actively prompt the user to install HidHide if it's missing. Once installed, the application automatically whitelists itself and seamlessly hides the hardware controller in the background.
...
* **DualSense Audio Haptic Feedback Support:** Introduced the new USBIP-based PS5 emu mode, which can establish a 4-channel audio playback device to receive DualSense audio haptic feedback from supported games. This architecture unifies PS5 Audio Haptics, Traditional PS5 Rumble, and Xbox Rumble. The pipeline features a highly optimized Adaptive Isochronous Pacer and robust signal isolation to ensure flawless, dropout-free haptic fidelity. Translated DualSense Adaptive Trigger signals to generate independent HD Rumble pulses, which specifically trigger upon physical trigger press (ZL/ZR) or when a payload change occurs while the physical trigger is actively held.
* **Audio Haptics Settings:** Independent toggle settings for the Audio Haptics End Point and Adaptive Trigger vibration feedback to customize the haptic experience in USBIP PS5 Emu Mode.
...
* **In-app Gyro Control (Mouse, R Joystick & Steering):** Utilize in-app gyro control for mouse aiming, or select R Joystick and Steering options. The Steering mode reads the controller's absolute tilt (accelerometer) and maps it directly to the Left Analog Stick's X-axis. Unlike the Mouse control mode, the R Joystick and Steering modes utilize a separate In-app Gyro mapping scope where all buttons default to their standard controller inputs to prevent unexpected mouse clicks during controller emulation. A dedicated Gyro Deadzone slider is also included to filter out unintentional micro-movements.
* **In-App Gyro Trigger Dampening:** A Trigger Dampening feature for the In-app Gyro allows setting a target trigger (ZL, ZR, or Both) and a dampening percentage. When the specified trigger is held, the gyro sensitivity is proportionally reduced (e.g., a 90% dampening amount reduces sensitivity to 10% of its original value) for precise aiming.
...
* **Magnetometer Calibration:** **Mag Cal** button for 9-axis accuracy. Perform a "figure-8" motion to calibrate the magnetometer (with a [quick link](https://youtu.be/J_cZnPcW-Yw?si=QWSizI49NQ_5OkA7) to a video tutorial).
* **Joystick Calibration:** **Joysticks Cal** button for in-app joystick calibration, integrated directly into the standard controller calibration process.
    1. **Rotate Stage (10s):** Fully rotate both joysticks. The system tracks active movement to accurately record the absolute maximum and minimum raw values.
    2. **Release Stage (3s):** Release the joysticks completely. The system monitors for strict stillness and collects an array of idle center samples.
...
* **Full Controller Remappability:** All buttons (including extra buttons like GL, GR, SL_L, SL_R, SR_L, SR_R, NSO GCN L/R Analog Trigger Click, Home, Capture, and Chat) can be fully remapped to Switch inputs, PlayStation inputs, In-app controls, or Windows controls (now including Media Keys, Task Manager, and On-Screen Keyboard). Joysticks can also be remapped to L/R Joystick, WASD, Keyboard Arrow Keys, mouse controls, or custom inputs. You can even assign "None" to a button to prevent any output. The mapping interface features a categorized pop-up window to streamline the selection process.
