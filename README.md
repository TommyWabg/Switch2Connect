...
* **DualSense Audio Haptic Feedback Support:** Introduced the new USBIP-based PS5 emu mode, which can establish a 4-channel audio playback device to receive DualSense audio haptic feedback from supported games. The PS5 configuration profile has been split into ps5_winuhid and ps5_usbip to manage driver-specific rumble settings independently.
  * ***To prevent Windows from outputting system audio to the controller, navigate to Windows 11 Settings > System > Sound > More sound settings > Playback tab, right-click the "Wireless Controller" device, and select "Disable".***
